...
func makeTea(arg int) error {
	if arg == 2 {
		...
		// We can wrap errors with higher-level errors to add context. The simplest way to do this is
		// with the %w verb in fmt.Errorf. Wrapped errors create a logical chain (A wraps B, which
		// wraps C, etc.) that can be queried with functions like errors.Is and errors.As.
...
func main() {
	...
	for i := range 5 {
		if err := makeTea(i); err != nil {
			// errors.Is checks that a given error (or any error in its chain) matches a specific error
			// value. This is especially useful with wrapped or nested errors, allowing you to identify
